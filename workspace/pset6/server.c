...
char* indexes(const char* path)
{
    char* path_match = malloc(strlen(path) + 11);
    if (path_match == NULL)
    {
        error(418);
        return NULL;
    }
    strcpy(path_match, path);
    strcat(path_match, "index.php");
    if (access(path_match, F_OK) == 0)
        return path_match;

    path_match[0] = '\0';
    strcpy(path_match, path);
    strcat(path_match, "index.html");
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while (true)
    {
        // read from file
        BYTE buffer[BYTES];
        ssize_t bytes = fread(buffer, 1, BYTES, file);
        if (bytes < 0)
        {
            if (*content != NULL)
            {
...
const char* lookup(const char* path)
{
    ...
    char* needle[8] = {".css", ".html", ".gif", ".ico", ".jpg", ".js", ".php", ".png"};
    for (int i = 0, j = sizeof(needle) / sizeof(char*);i < j ;i++)
    {
        char* ptr = strcasestr(path, needle[i]);
        if (ptr != NULL)
        {
            switch (i)
            {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    if (ptr != NULL)
    {
        ...
        strncpy(abs_path, request_target, ptr - request_target);
        abs_path[ptr - request_target] = '\0';
    }
    // there was no query
    else
    {
